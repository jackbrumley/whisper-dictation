...
running = True
restart = False
input_ready = True  # Tracks if a message about input readiness needs to be printed
...
# Disable Tkinter's geometry persistence
os.environ['TK_NO_CFG'] = '1'
...
def main():
  ...
  def show_status_window(status):
    ...
    # Window dimensions
    window_width = 250  # Reduced width for smaller size
    window_height = 80  # Reduced height for smaller size
    x_position = (screen_width // 2) - (window_width // 2)  # Centered horizontally
    y_position = screen_height - window_height - PIXELS_FROM_BOTTOM  # Adjusted for taskbar height
    ...
    # Add label
    label = tk.Label(window, text=status, font=("Helvetica", 16), bg="white")  # Set background to white
    label.pack(expand=True, fill=tk.BOTH)  # Ensure label fills the window

    # Explicitly set the position again
  ...
  def dictation_process():
    ...
    while running:
      if input_ready:
        print(f"Waiting for input. Press and hold {KEYBOARD_SHORTCUT} to start recording...")  # Debug
        input_ready = False
      if keyboard.is_pressed(KEYBOARD_SHORTCUT):
        print("Shortcut detected, starting dictation process...")  # Debug
        input_ready = False  # Reset to prevent re-printing during active process
        ...
        input_ready = True  # Ready for new input
      else:
        time.sleep(0.1)
